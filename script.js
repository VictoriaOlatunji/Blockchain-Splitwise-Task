...
async function getNeighbors(node){ // get neighbor of a node (addr, amount) in debtor-creditor(IOU) map
	let returnedLedger = await getLedger();
	var idx = 0; 
	var debtorExist = false;
	var neighbors = [];
	for (var i = 0; i < returnedLedger.length; i++){ // find for which node we're finding its neighbors
		if (returnedLedger[i]["debtor"].toLowerCase() == node.toLowerCase()){
			idx = i;
			debtorExist = true;
			break;
		}
	...
	if (debtorExist){
		for (var j = 0; j < returnedLedger[idx]["IOUs"].length; j++){ 
			if (returnedLedger[idx]["IOUs"][j]["_valid"])
				neighbors.push({"creditor": returnedLedger[idx]["IOUs"][j]["creditor"], "amount": returnedLedger[idx]["IOUs"][j]["amount"]});
		}
	} 
	return neighbors;
}
...
async function getUsers() {
	let returnedLedger = await getLedger();
	var users = new Set();
	for (var i = 0; i < returnedLedger.length; i++){
		users.add(returnedLedger[i].debtor);
		for (var j = 0; j < returnedLedger[i].IOUs.length; j++){ 
			users.add(returnedLedger[i]["IOUs"][j]["creditor"]);
		}
	}
